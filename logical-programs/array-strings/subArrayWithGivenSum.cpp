...
returns an ArrayList containing the starting and ending positions 
...
using namespace std;
/*
simple solution , with time complexity O(N*N)
*/
void printSubArrayWithGivenSum(int A[], int N, int S){
    int currentSum = 0;
    for(int i = 0; i < N; i++){
        currentSum = A[i];
        if(currentSum == S){
            cout << "Sum Found at index " << i+1 << " Single element "<< endl;
            return;
        }
        for(int j = i + 1; j < N; j++){
            currentSum += A[j];
            if(currentSum == S){
                cout << "Sum Found at index " << i+1 << " " << j+1 << " " << endl;
                return;
            }
        }
    }
    cout << "There is no subarray that sums to " << S << endl;
}
